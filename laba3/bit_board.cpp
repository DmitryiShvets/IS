...
Position::Position() : board(0), count_queens(0) {}

Position::Position(std::bitset<N* N> _board, uint8_t _count_queens) : board(_board), count_queens(_count_queens) {}
...
std::vector<int> Position::to_vector() const {
...
void Position::place_queen(int x, int y) {
	board[y * N + x] = 1;
}

bool Position::is_safe(int x, int y) const {
...
std::bitset<N* N> Position::get_new_pos(int x, int y) const {
	auto tmp = board;
	tmp[y * N + x] = 1;
	return tmp;
}
...
PositionN PositionN::mutate(double mutation_rate) const {
	auto queens = pos;
...
PositionN PositionN::crossover(const PositionN& other) const{
